...
The dpe module is the main entry point for dpe functionality.
To use it, one must `require('dpe')`.

## dpe.context([config])

Creates and returns a new context which represents the connection
to the dpe cluster, and which can be used to create datasets on that
cluster. Config is an *Object* which defines the cluster server,
with the following defaults:

```javascript
{
  host: 'localhost',  // Cluster server host, settable also by dpe_HOST env
  port: '12346'       // Cluster server port, settable also by dpe_PORT env
}
```
...
Dataset objects, as created initially by above dpe context source
...
```javascript
var hp = new dpe.HashPartitioner(3)
...
var rp = new dpe.RangePartitioner(3, a => a, dataset)
var dataset = sc.range(10).partitionBy(rp)
```

## Environment variables

- `dpe_HOST`: The hostname of the dpe-server process in distributed mode. If unset, the master runs in standalone mode.
- `dpe_PORT`: The port of the dpe-server process in distributed mode. Default value: "12346"
- `dpe_KEY`: An authentication token which may be required by the dpe-server process
- `dpe_DEBUG`: set the debug trace level to the following values:
