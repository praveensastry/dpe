...
function Client(opt = {}, callback) {
  ...
  if (!opt.ws) opt.ws = inBrowser ? true : process.env.dpe_WS;
  if (!opt.host && !inBrowser) opt.host = process.env.dpe_HOST;
  if (!opt.port && !inBrowser) opt.port = process.env.dpe_PORT;
  if (!opt.debug && !inBrowser) opt.debug = process.env.dpe_DEBUG;
  if (!opt.access && !inBrowser) opt.access = process.env.dpe_KEY;
