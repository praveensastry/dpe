...
const help = 'Usage: dpe [options] <command> [<args>]\n' +
'\n' +
...
'  deploy [<args>...]  Deploy application on dpe cloud\n' +
'  run [<args>...]     Run application on dpe cloud\n' +
'  attach              Attach to a running application\n' +
'  list                List deployed applications\n' +
'  log                 Print log of an application\n' +
'  signup              Create an account on dpe cloud\n' +
'  status [<name>]     Print status of application on dpe cloud\n' +
'  stop                Stop application on dpe cloud\n' +
'\n' +
'Options:\n' +
...
if (argv.d || argv.debug) {
  process.env.dpe_DEBUG = 2;
}

const configPath = argv.c || argv.config || process.env.dpe_CONFIG || process.env.HOME + '/.dperc';
const config = load_config(argv);
const rc = netrc();
const start = process.hrtime();
let trace;

if (process.env.dpe_DEBUG > 1) {
  trace =  function trace() {
    const args = Array.prototype.slice.call(arguments);
    const elapsed = process.hrtime(start);
    args.unshift('[dpe ' + (elapsed[0] + elapsed[1] / 1e9).toFixed(3) + 's]');
...
function create(name) {
  if (!checkName(name)) die('dpe create error: invalid name ' + name);
  console.log('create application ' + name);
  try {
    fs.mkdirSync(name);
  } catch (error) {
    die('dpe create error: ' + error.message);
  }
  process.chdir(name);
  ...
  try {
    ...
    console.log('It looks like you may not have git installed.  dpe needs that to run.  See https://git-scm.com/ for installation.');
    return;
  }
  ...
  const src = '#!/usr/bin/env node\n' +
    '\n' +
    'const sc = require(\'dpe-engine\').context();\n' +
    '\n' +
  ...
  if (npm.status) die('dpe create error: npm install failed');


  console.log('\n----------------------------------' +
    '\nProject ' + name + ' is now ready!\n' +
    'Please change directory to ' + name + ': "cd ' + name + '"\n' +
    'To run your app locally: "dpe test"\n' +
...
function load_config(argv) {
  ...
  process.env.dpe_TOKEN = process.env.dpe_TOKEN || conf.token;
...
function dpe_session(callback) {
  const host = process.env.dpe_SERVER || 'apps.dpe.me';
  const port = process.env.dpe_PORT || 443;

  const ddp = new DDPClient({
    // All properties optional, defaults shown
    host : host,
    port : port,
    ssl  : !process.env.dpe_NOSSL,
    autoReconnect : true,
  ...
  ddp.connect(function (err, isreconnect) {
    ...
    login(ddp, {env: 'dpe_TOKEN', retry: 2}, function (err, userInfo) {
      if (err) return callback(err, ddp, isreconnect);
...
function deploy() {
  dpe_session(function (err, ddp) {
    ...
    ddp.call('etls.add', [{name: name}], function (err, res) {
      ...
      netrc.save(rc);
      child_process.exec('git remote remove dpe; git remote add dpe "' + res.url + '"; git add -A .; git commit -m "automatic commit"; git pull --rebase -Xours dpe master; git push dpe master', function (err) {
...
function run_remote() {
  try {
    const diff = child_process.execSync('git diff dpe/master', {stdio: ['pipe', 'pipe', 'ignore']});
    if (diff.length) {
    ...
    die('This application is not deployed. Run first "dpe deploy"');
  }
  dpe_session(function (err, ddp) {
    if (err) die('Could not connect:', err);
    const pkg = JSON.parse(fs.readFileSync('package.json'));
    const name = pkg.name;
    const opt = {debug: process.env.dpe_DEBUG};
    let ltrace;

    trace('run triggered, wait for machine');
    ddp.call('etls.run', [{name: name, opt: opt}], function (err, res) {
      if (err) die('run error:', err);
      if (res.alreadyStarted) die('Error: application is already running, use "dpe attach" or "dpe stop"');
...
function attach() {
  dpe_session(function (err, ddp) {
    ...
    ddp.subscribe('etls.withName', [name], function () {
      ...
      if (!etl.running) die('Application is not running, use "dpe log" or "dpe run"');
  ...
  dpe_session(function (err, ddp) {
    ...
    if (!name) {
      try {
        ...
        die('Could not find package.json.  You need to run this command from a dpe project directory.');
      }
    }
