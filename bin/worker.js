...
const nworkers = +opt.options.nworker || +(process.env.dpe_WORKER_PER_HOST ? process.env.dpe_WORKER_PER_HOST : os.cpus().length);
const memory = +(opt.options.memory || process.env.dpe_MEMORY);
...
if (cluster.isMaster) {
  process.title = 'dpe-worker-controller';
  if (memory)
  ...
  setInterval(function () {
    ...
    fs.writeFile('/tmp/dpe/worker-controller-stats', JSON.stringify(stats), function () {});
  }, 3000);
...
function runWorker(host, port) {
  ...
  if (process.env.dpe_RANDOM_SEED)
    Dataset.setRandomSeed(process.dpe_RANDOM_SEED);
  process.on('uncaughtException', function (err) {
    grid.send(grid.muuid, {cmd: 'workerError', args: err.stack});
    process.exit(2);
  });

  const grid = new dpeClient({
    debug: debug,
    host: host,
    port: port,
    data: {
    ...
    process.title = 'dpe-worker_w' + res.id;
  });
