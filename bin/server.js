...
const dpeClient = require('../lib/client.js');
...
const clients = {};
let clientNum = 1;
const clientMax = dpeClient.minMulticast;
const minMulticast = dpeClient.minMulticast;
const topics = {};
let topicNum = -1;
const UInt32Max = 4294967296;
const topicMax = UInt32Max - minMulticast;
const topicIndex = {};
const memory = opt.options.memory || process.env.dpe_MEMORY || 0;
...
const access = process.env.dpe_KEY;
const stats = {
  masters: 0,
  workerControllers: 0,
  workers: 0,
...
SwitchBoard.prototype._transform = function (chunk, encoding, done) {
  ...
  if (to >= minMulticast) { // Multicast
    ...
    if (!(o.cmd in clientRequest)) {
      ...
      o.cmd = 'reply';
      this.sock.write(dpeClient.encode(o), done);
...
const clientRequest = {
  connect: function (sock, msg) {
    ...
    if (access && msg.access !== access) {
      console.log('## dpe connect failed: access denied');
      msg.error = 'access denied, check dpe_KEY';
      return true;
    }
    ...
    if (msg.data.notify in clients && clients[msg.data.notify].sock) {
      clients[msg.data.notify].sock.write(dpeClient.encode({cmd: 'notify', data: msg.data}));
      ...
      if (wsid === msg.data.wsid) {
        ...
        if (pendingMasters.length && workerStock.length >= expectedWorkers) {
          ...
          if (clients[master.data.uuid].sock)
            clients[master.data.uuid].sock.write(dpeClient.encode(master));
...
function startWorkerStock() {
  ...
  for (let i = 0; i < workerControllers.length; i++) {
    clients[workerControllers[i].uuid].sock.write(dpeClient.encode({
      cmd: 'getWorker',
      wsid: wsid,
...
function pubmon(data) {
  mstream.write(dpeClient.encode({cmd: 'monitoring', id: monid, data: data}));
}
...
fs.mkdir('/tmp/dpe', function () {});
setInterval(function () {
  fs.writeFile('/tmp/dpe/server-stats', JSON.stringify(stats), function () {});
}, 3000);
  ...
  if (cli) {
    ...
    for (let i in cli.closeListeners) {
      if (i in clients && clients[i].sock)
        clients[i].sock.write(dpeClient.encode({cmd: 'remoteClose', data: cli.uuid}));
    }
...
function handleConnect(sock) {
  ...
  sock.pipe(new dpeClient.FromGrid()).pipe(new SwitchBoard(sock));
}
